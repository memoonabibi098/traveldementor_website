   let counterIndex = 1;
    ...
    // Add counter in edit modal
    if (e.target.classList.contains('add-counter-edit')) {
        const wrapper = e.target.previousElementSibling;
        let index = wrapper.querySelectorAll('.counter-row').length;
        ...
        row.innerHTML = `
            <div class="col-md-3">
                <input type="number" name="counters[${index}][value]" class="form-control" required>
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-2">
                <input type="number" name="counters[${index}][order]" class="form-control" value="1">
            </div>
            <div class="col-md-1">
                <button type="button" class="btn btn-danger remove-counter">×</button>
            </div>
    ...
    // Remove counter (works for both create & edit)
    if (e.target.classList.contains('remove-counter')) {
        e.target.closest('.counter-row').remove();
    }
});
